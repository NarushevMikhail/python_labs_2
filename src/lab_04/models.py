...
class User(Profile, Authentication, Printable, Comparable):
    total_users = 0
    ...
    def check_password(self, password: str): #добавил два метода, т к наследуют от басьрактного метода классы 
    ...
    def print_str(self):
        return f'Имя пользователя: {self.nickname}, его роль: {self.role}'
    
    def compare_to(self, other): #сравнение по nickname 
        if not isinstance(other, User):
            return 1
...
class Guest(Profile, Printable, Comparable): #второй класс
    def __init__(self, session_id: str = "default"):
        self._session_id = session_id
        self._nickname = "Гость"
        self._login = f"guest_{session_id}"
        self._role = "guest"
        self._bio = "Гостевой доступ"
        self._age = 0
        self._city = "Не указан"
    ...
    @nickname.setter
    def nickname(self, value):
        raise PermissionError('Гости не могут изменять nickname')
    ...
    @login.setter
    def login(self, value):
        raise PermissionError("Гостевой аккаунт не может изменять login")
    ...
    @role.setter
    def role(self, value):
         raise PermissionError("Гостевой аккаунт не может изменять роль")
    
    @property #всегда возвращает защищеный атрибут 
    def session_id(self):
    ...
    def session_id(self, value):
        if isinstance(value, str):
            ...
            raise TypeError("session_id должна быть строкой")
    
    # Методы из интерфейса Profile
    def update_profile(self, bio = None, age = None, city = None):
        raise PermissionError("Гостевой аккаунт не может изменять профиль") 
    ...
    def view_profile(self):
        return {
            ...
            'age': self._age,
            'city': self._city,
            'note': 'Зарегистрируйтесь для полного доступа'
        }
    
    def __str__(self):
        return f"Гость (сессия: {self.session_id})"
    
    def print_str(self):
        return f'Имя гостя: {self.nickname}, номер сессии: {self.session_id}'
